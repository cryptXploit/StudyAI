...
export class GeminiAdapter implements ProviderAdapter {
  ...
  private getSafeModelName(model: string): string {
    ...
    if (lower === 'pro') return process.env.DEFAULT_GEMINI_COMPLEX_MODEL || 'gemini-3.1-pro-preview';
    
    // If the admin typed an exact google model name, trust it
    if (lower.startsWith('gemini-')) return model;
    
    // Absolute Default
    return process.env.DEFAULT_GEMINI_GENERAL_MODEL || 'gemini-3.5-flash';
  }
  ...
  async generateEmbedding(text: string, model: string): Promise<number[]> {
    ...
    // Fallback to gemini-embedding-2 if model is not properly passed
    const actualModel = model ? model : 'gemini-embedding-2';
