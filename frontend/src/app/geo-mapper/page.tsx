...
const translations = {
  English: {
    ...
    mapper: "Mapper",
    chat: "Chat",
    enterTopic: "Enter a topic to generate an interactive world map.",
    legend: "Legend",
    dataCsv: "Data CSV",
    generatedBy: "Generated by Prepia",
    dataColorMatch: "Data Color Match",
  ...
  Bangla: {
    ...
    queryTopic: "অনুসন্ধান টপিক",
    mapper: "ম্যাপার",
    chat: "চ্যাট",
    enterTopic: "একটি ইন্টারঅ্যাকটিভ ওয়ার্ল্ড ম্যাপ তৈরি করতে একটি টপিক লিখুন।",
    legend: "লিজেন্ড",
    dataCsv: "ডেটা CSV",
    generatedBy: "Prepia দ্বারা তৈরি",
    dataColorMatch: "ডেটা কালার ম্যাচ",
    aiInsights: "এআই অন্তর্দৃষ্টি",
    topCountries: "শীর্ষ দেশসমূহ",
    query: "অনুসন্ধান",
    stats: "পরিসংখ্যান"
  },
  Hindi: {
    ...
    mapper: "मैपर",
    chat: "चैट",
    enterTopic: "इंटरैक्टिव वर्ल्ड मैप बनाने के लिए एक विषय दर्ज करें।",
    legend: "संकेत",
    dataCsv: "डेटा CSV",
    generatedBy: "Prepia द्वारा निर्मित",
    dataColorMatch: "डेटा रंग मिलान",
...
export default function GeoMapperPage() {
  ...
  const renderSearchSection = () => (
    <form onSubmit={(e) => { handleGenerateMap(e); setIsMobileDrawerOpen('none'); }} className="space-y-4 bg-slate-950 p-4 md:p-5 rounded-2xl border border-slate-800 shadow-inner shrink-0">
      <div>
        <label className="block text-xs font-black tracking-widest text-slate-400 uppercase mb-2 flex items-center gap-1.5"><Search size={14}/> {t.queryTopic}</label>
      ...
      <div className="flex flex-col md:flex-row h-[calc(100vh-80px)] max-w-7xl mx-auto overflow-hidden bg-slate-950 lg:border lg:border-slate-800 lg:rounded-3xl lg:shadow-2xl lg:mt-4 font-sans relative">
        ...
        <div className={`lg:hidden h-[60px] mx-3 mt-3 rounded-2xl flex items-center justify-between px-4 z-40 sticky backdrop-blur-2xl shadow-lg transition-all duration-300 border ${isHeaderVisible ? 'top-3 opacity-100 translate-y-0' : '-top-20 opacity-0 -translate-y-full'} bg-slate-900/90 border-slate-700/50 shadow-[0_0_15px_rgba(0,0,0,0.2)] shrink-0`}>
          <div className="flex flex-col">
            <h2 className="text-lg font-black tracking-tight flex items-center gap-2 uppercase text-slate-100"><Globe size={16} className="text-cyan-500"/> {t.mapper}</h2>
            <p className="text-[9px] font-bold text-slate-400 flex items-center gap-1.5 uppercase tracking-widest">{t.title}</p>
          </div>
          <button onClick={() => window.location.href='/chat'} className="px-3 py-1.5 font-black rounded-lg transition uppercase tracking-wider text-[10px] bg-cyan-600 text-slate-900 shadow-md hover:bg-cyan-500">{t.chat}</button>
        </div>
        ...
        <div onScroll={handleScroll} className="flex-1 relative bg-[#020617] flex flex-col items-center justify-center overflow-y-auto custom-scrollbar">
          ...
          {!mapData && !isLoading ? (
            <div className="text-center opacity-50">
               ...
               <p className="text-lg font-bold text-slate-400">{t.enterTopic}</p>
            </div>
          ) : mapData && (
            <div ref={mapContainerRef} className="w-full h-full min-h-[500px] md:min-h-0 relative flex flex-col p-2 md:p-6 pb-24 md:pb-12 animate-in zoom-in-95 duration-500">
              ...
              {mapData.legend && mapData.legend.length > 0 && (
                <div className="absolute bottom-24 md:bottom-8 left-4 md:left-8 z-20 bg-slate-900/80 backdrop-blur-md border border-slate-700 p-3 md:p-4 rounded-xl md:rounded-2xl shadow-xl pointer-events-none">
                  <h4 className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 md:mb-3">{t.legend}</h4>
              ...
              <div className="absolute top-4 right-4 md:top-8 md:right-8 z-20 flex flex-col md:flex-row items-end md:items-center gap-2">
                <button onClick={exportAsCSV} className="flex items-center gap-2 bg-slate-800/80 hover:bg-slate-700 backdrop-blur-md border border-slate-600 text-white p-2 md:px-4 md:py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all shadow-lg active:scale-95 group">
                  ...
                  <span className="hidden md:inline">{t.dataCsv}</span>
                </button>
              ...
              <div className="absolute bottom-24 md:bottom-6 right-4 md:right-8 z-20 opacity-40 pointer-events-none">
                <p className="text-[8px] md:text-[10px] font-black tracking-widest text-white uppercase">{t.generatedBy}</p>
              </div>
              ...
              {selectedCountryId && mappedCountries[selectedCountryId] && (
                <div className="absolute top-32 md:top-auto md:bottom-12 left-1/2 transform -translate-x-1/2 z-40 bg-slate-900/95 backdrop-blur-xl border border-cyan-500/50 p-4 rounded-2xl shadow-[0_0_40px_rgba(34,211,238,0.3)] animate-in slide-in-from-top-10 md:slide-in-from-bottom-10 w-[85%] md:w-auto min-w-[300px] max-w-md flex flex-col gap-3 pointer-events-auto">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t.dataColorMatch}</span>
                  </div>
                </div>
              )}
        ...
        {mapData && (
          <div className="hidden lg:flex lg:w-72 xl:w-80 bg-slate-900 border-l border-slate-800 p-4 lg:p-6 flex-col z-10 shrink-0 h-full overflow-y-auto custom-scrollbar">
            <h3 className="text-xs font-black tracking-widest text-cyan-400 uppercase mb-3 flex items-center gap-2"><Lightbulb size={14}/> {t.aiInsights}</h3>
            ...
            <h3 className="text-xs font-black tracking-widest text-slate-400 uppercase mb-3 flex items-center gap-2"><TrendingUp size={14}/> {t.topCountries}</h3>
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 custom-scrollbar pb-6">
